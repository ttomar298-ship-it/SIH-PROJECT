...
try:
    ...
    from utils.gov_theme import apply_gov_theme, hide_default_sidebar_nav, render_citizen_navbar, render_gov_footer
except ImportError:
    from frontend.streamlit_app.utils.api_client import client
    from frontend.streamlit_app.utils.gov_theme import apply_gov_theme, hide_default_sidebar_nav, render_citizen_navbar, render_gov_footer

try:
    from auth_guards import require_citizen_login
except ImportError:
    from frontend.streamlit_app.auth_guards import require_citizen_login
...
# Apply GOI base theme CSS (palette, fonts, status pills, footer).
apply_gov_theme()
hide_default_sidebar_nav()
render_citizen_navbar(logo_path=LOGO_PATH)
require_citizen_login()

# ── Citizen logout button (scoped: clears ONLY citizen_* keys) ────────────
_logout_col, _spacer = st.columns([1, 6])
with _logout_col:
    if st.button("🚪 Citizen Logout", key="_citizen_logout_top", help="Sign out of Citizen Tracker"):
        for _k in ["citizen_authenticated", "citizen_identifier",
                   "citizen_otp", "citizen_otp_ts", "citizen_pending_id"]:
            st.session_state.pop(_k, None)
        if "citizen_session" in st.query_params:
            del st.query_params["citizen_session"]
        if "citizen_id" in st.query_params:
            del st.query_params["citizen_id"]
        st.rerun()
