...
logger = logging.getLogger("bhoomi_ai.officer_dashboard")
...
# ── Page Config ───────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Officer Dashboard — BHOOMI AI",
    page_icon="🏛️",
    layout="wide",
...
apply_gov_theme()
hide_default_sidebar_nav()
render_top_navbar(current_slug="Officer_Dashboard", logo_path=LOGO_PATH)
...
welcome_msg = f"Welcome back, <strong>{curr_user['name']}</strong> ({curr_user['role']})" if curr_user else "Welcome, Authorized Officer"
...
with c_refresh_btn:
    if st.button("🔄 Refresh Data", key="btn_refresh_officer_dashboard", help="Reload latest cached records"):
        st.cache_data.clear()
        st.rerun()
...
st.write("")
st.info("💡 **Officer Tools:** Navigate above to view **Project Details**, **Risk Rankings**, **SHAP AI Explanations**, **GIS Map**, **Alerts**, or the **DILRMP Digital Land Records Registry**.")
